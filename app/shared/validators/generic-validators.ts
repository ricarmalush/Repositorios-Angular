    ...
    static validateEmail(control: AbstractControl): { [key: string]: boolean } | null {
        if (control.value != null && control.value.toString().trim() != "") {
            ...
            if (emailPattern.test(control.value)) {
                return null; // Si es un correo electrónico válido, devuelve null
            } else {
                return { 'invalidEmail': true }; // Si no es un correo electrónico válido, devuelve un error
            }
        } else {
            return null; // Si el campo está vacío o nulo, no se realiza ninguna validación
        }
    }
