...
import { componentSettings } from './province-list-config';
...
export class ProvinceListComponent implements OnInit {
  ...
  rowClick(e: any) {
    ...
    switch (action) {
      case "edit":
        ...
        break;
      case "remove":
        this.ProvinceRemove(province);
        break;
    }
    return false;
  }

  ProvinceEdit(row: ProvinceResponse) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.data = row;

    let dialogRef = this._dialog.open(ProvinceManageComponent, {
      data: dialogConfig,
      disableClose: true,
      width: "400px",
    });
  ...
  ProvinceRemove(province: any) {
    Swal.fire({
      title: `¿Realmente deseas eliminar la provincia ${province.provinceName}?`,
      ...
      if (result.isConfirmed) {
        this._provinceService.ProvinceRemove(province.provinceId)
        .subscribe({
          next: (response) => {
  ...
  get getDownloadUrl() {
    return `Provinces?Download=True`;
  }
