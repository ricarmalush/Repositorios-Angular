...
import { componentSettings } from './role-config';
...
export class RoleListComponent implements OnInit {
  ...
  RoleEdit(row: RoleResponse) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.data = row;

    let dialogRef = this._dialog.open(RoleManageComponent, {
      data: dialogConfig,
      disableClose: true,
      width: "400px",
    });
  ...
  RoleRemove(role: any) {
    Swal.fire({
      title: `¿Realmente deseas eliminar el role ${role.name}?`,
      ...
      if (result.isConfirmed) {
        this._roleService.RoleRemove(role.roleId)
        .subscribe({
          next: (response) => {
