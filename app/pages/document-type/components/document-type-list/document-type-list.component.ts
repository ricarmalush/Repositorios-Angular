...
import { componentSettings } from './document-type-config';
...
export class DocumentTypeListComponent implements OnInit {
  ...
  DocumentTypeEdit(row: DocumentTypeResponse) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.data = row;

    let dialogRef = this._dialog.open(DocumentTypeManageComponent, {
      data: dialogConfig,
      disableClose: true,
      width: "400px",
    });
  ...
  DocumentTypeRemove(documentType: any) {
    Swal.fire({
      title: `¿Realmente deseas eliminar el documento ${documentType.name}?`,
      ...
      if (result.isConfirmed) {
        this._documentTypeService.DocumentTypeRemove(documentType.documentTypeId)
        .subscribe({
          next: (response) => {
